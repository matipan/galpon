...
local function replace_comment(text, save)
  local suffix = save and "<Esc><CR>" or "<Esc>"
...
local colors = {
  ...
  Status = "#7799ff", StatusInk = "#101010", Blue = "#7799aa",
  Cyan = "#66aacc", Purple = "#aa88cc", Green = "#aadd88",
  Orange = "#dd9955", Red = "#dd6677", Yellow = "#ddbb66", Teal = "#55bbaa",
}
...
if vim.env.GALPON_REVIEW_TEST_RUNTIME and vim.env.GALPON_REVIEW_TEST_RUNTIME ~= "" then
  ...
  local function rendered_marks()
    return #api.nvim_buf_get_extmarks(state.source_buffer, renderer_namespace, 0, -1, {})
  end
  wait_for(function() return rendered_marks() > 0 end, "Normal mode did not render Markdown")
  keys("v")
  wait_for(function() return rendered_marks() == 0 end, "Visual mode did not expose raw Markdown")
  equal(table.concat(api.nvim_buf_get_lines(state.source_buffer, 0, -1, true), "\n"), text, "rendering changed source text")
  keys("<Esc>")
  wait_for(function() return rendered_marks() > 0 end, "Normal mode did not restore rendered Markdown")
...
-- Both native controls and Markdown use the supplied palette, not Neovim defaults.
local function highlight(name, field, color)
  equal(api.nvim_get_hl(0, { name = name, link = false })[field], tonumber(color:sub(2), 16), name .. " did not use the supplied palette")
end
for _, group in ipairs({ "LineNr", "NonText", "Conceal" }) do highlight(group, "fg", colors.Comment) end
highlight("CursorLineNr", "fg", colors.Blue)
highlight("Pmenu", "bg", colors.Prompt)
highlight("PmenuSel", "bg", colors.Selection)
highlight("MsgArea", "bg", colors.Background)
highlight("ErrorMsg", "fg", colors.Red)
highlight("WarningMsg", "fg", colors.Yellow)
highlight("@markup.heading.1.markdown", "fg", colors.Blue)
...
if state.render_markdown then
  ...
  highlight("RenderMarkdownCodeInline", "bg", colors.Prompt)
  highlight("RenderMarkdownBullet", "fg", colors.Teal)
end
...
-- Normal c captures a logical line. Insert Enter adds a line; Esc then Enter saves.
...
for _, mode in ipairs({ "n", "i" }) do
  expect(vim.fn.maparg("<C-s>", mode, false, true).buffer ~= 1, "Review must not bind Ctrl-s in a comment")
end
expect(vim.fn.maparg("<CR>", "i", false, true).buffer ~= 1, "Insert Enter must keep native newline editing")
replace_comment("First line<CR>Second line", false)
expect(state.comment ~= nil and #state.items == 1, "Insert Enter saved instead of adding a line")
equal(table.concat(api.nvim_buf_get_lines(state.comment.buffer, 0, -1, true), "\n"), "First line\nSecond line", "Insert Enter did not add a newline")
...
wait_for(function() return state.comment == nil end, "Esc then Enter did not save and close the comment buffer")
...
wait_for(function() return state.comment == nil end, "Esc then Enter did not finish annotation editing")
