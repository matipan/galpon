...
This directory vendors a fork of `@juicesharp/rpiv-todo` 2.7.1. Galpón loads it for every managed Pi agent. It keeps the upstream tool, replay, and overlay behavior and adds versioned Pi event-bus contracts under `integrations/`. Its Pi-local readiness selector reports an incomplete task as ready and unassigned only when all blockers are complete, the owner is empty, all linked delegations are settled, and no associated daemon operation owns it. The overlay shows an exact bounded `N ready` value only after daemon reconciliation. It shows `ready unknown` when replay input is stale, reconciliation fails, or more than 256 local associations exist. This is visibility only. It does not claim or schedule work. TODO subjects and snapshots stay in Pi.

The task model has no general causal field that links an arbitrary Pi operation to a TODO. After the reducer commits, the TODO tool therefore emits a bounded local result with only the action, applicable task ID and final status, and a `changed`, `no_change`, or `rejected` effect. A changed update that leaves a task incomplete can create a durable local association with the current operation. Completion or deletion removes that task ID from every operation. Clear removes every association, and these global removals replay in order so a reused task ID can associate again. Every daemon-authoritative nonterminal state (`ready`, `claimed`, `running`, `waiting`, or `settling`) owns its associated tasks. Thus, a parked operation remains an owner. A runtime- and generation-fenced daemon query removes missing or terminal owners after reload, periodically, after settlement, and after recovery. The overlay event carries task IDs and exact/unknown knowledge only. It never carries operation IDs, TODO subjects, or TODO snapshots.
